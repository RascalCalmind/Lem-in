...
/*   Updated: 2020/04/20 16:36:46 by wmisiedjan    ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_handle_command_line(t_lemin *lemin, t_rstr *file)
{
		...
		ft_strcmp(file->str, "##end") == 0)
	{
		...
		if (!file->str)
			return (ft_free_error(lemin, file, ERR_START_END));
		while (file->str[0] == '#')
			file = file->next;
...
static int		ft_read_ants(t_lemin *lemin, t_rstr *file)
{
	while (file->str[0] == '#' && file->str[1] != '#')
		file = file->next;
	if (ft_contains(file->str, ' ') || ft_check_int(file->str) != 0) {
		return (ERR_FILE);
	} else {
...
static int		ft_create_lists(t_lemin *lemin, t_rstr *file)
{
	...
	file = file->next;
	while (file != NULL && file->str != NULL)
	{
		if (file->str && file->str[0] == '#' && file->str[1] != '#')
			;
		else if (ft_is_command(file->str))
			ft_handle_command_line(lemin, file);
		else if (ft_contains(file->str, ' ') == 2)
		{
			ret = ft_store_room(lemin, file);
			if (ret <= 0)
				return (ft_free_error(lemin, file, ERR_PARSE_ROOM));
		}
...
static t_lemin	*ft_lem_in(t_rstr *file)
{
	...
	{
		ft_free_error(lemin, file, \
		lemin->rooms < 2 ? ERR_INVALID_ROOM_COUNT : ERR_MEM);
		return (NULL);
	}

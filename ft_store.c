...
/*   Updated: 2020/02/08 15:55:56 by lhageman      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
double		ft_calc_heuridian(t_lemin *list, t_room *room, char *str)
{
	...
	if (list->room[end] != NULL)
	{
		...
		{
			x = (double)room->x - (double)list->room[end]->x;
			ft_printf("HEURIDIAN x: %i\n", x);
			x *= x;
			ft_printf("HEURIDIAN x: %i\n", x);
			y = (double)room->y - (double)list->room[end]->y;
			ft_printf("HEURIDIAN y: %i\n", y);
			y *= y;
			ft_printf("HEURIDIAN y: %i\n", y);
			h = x + y;
			ft_printf("HEURIDIANNNN h: %i + %i = %i\n", x, y, h);
